...
WEATHER_DESCRIPTION_PT_BR=$(echo $WEATHER_JSON | jq '.current_condition[0].lang_pt-br[0].value' | tr -d '"' | sed 's/\(.\{16\}\).*/\1.../')

if [ -n "$WEATHER_DESCRIPTION_PT_BR" ]; then
  WEATHER_DESCRIPTION="$WEATHER_DESCRIPTION_PT_BR"
fi
