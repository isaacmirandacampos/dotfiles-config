-- Filename: ~/.config/neovim/neobean/lua/plugins/blink-cmp.lua
-- ~/.config/neovim/neobean/lua/plugins/blink-cmp.lua
...
return {
	...
	opts = function(_, opts)
		...
		opts.enabled = function()
			...
			if filetype == "fzf" or filetype == "minifiles" then
				return false
			end
			return true
		end
		...
		opts.sources = vim.tbl_deep_extend("force", opts.sources or {}, {
			...
			providers = {
				lsp = {
					...
					kind = "LSP",
					min_keyword_length = 0, -- Allow showing completions even after trigger chars like '.'
				...
				buffer = {
					...
					min_keyword_length = 2, -- Keep at 2 to avoid noise
				...
				dictionary = {
					...
					opts = {
						...
						dictionary_files = {
							...
							vim.fn.expand("~/.config/nvim/spell/pt.utf-8.add"),
						},
		...
		opts.completion = {
			...
			menu = {
				...
				draw = {
					-- Colunas a exibir em cada item
					columns = {
						{ "kind_icon" }, -- Ícone do tipo (função, variável, etc)
						{ "label", "label_description", gap = 1 }, -- Nome do item
						{ "source_name" }, -- Nome da fonte (LSP, Copilot, etc)
					},
					components = {
						source_name = {
							-- Mostrar de onde vem a sugestão
							text = function(ctx)
								...
								return source_mapping[ctx.source_id] or "[" .. ctx.source_name .. "]"
							end,
							highlight = function(ctx)
								-- Cores diferentes para cada fonte
								local source_hl = {
									lsp = "@comment.note", -- azul
									copilot = "@string", -- verde
									path = "@number", -- laranja
									buffer = "@comment", -- cinza
									snippets = "@keyword", -- roxo
									emoji = "@character", -- amarelo
									dictionary = "@function", -- azul claro
									dadbod = "@type", -- amarelo
								}
								return source_hl[ctx.source_id] or "@comment"
							end,
						},
					},
				},
		...
		opts.keymap = {
			-- Removed preset to avoid conflicts
			["<CR>"] = { "accept", "fallback" },
